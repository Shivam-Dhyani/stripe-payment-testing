...
const Carts = () => {
  ...
  if (loading) {
    return (
      <div>
        <h1 className="text-3xl font-bold text-slate-800 mb-6">User Carts</h1>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <h1 className="text-3xl font-bold text-slate-800">User Carts</h1>
          {carts.length > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm text-slate-500">Users with Carts</p>
              <p className="text-2xl font-bold text-slate-800">{carts.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800">${totalCartValue.toFixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {carts.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-slate-500 text-lg">No users have items in their cart</p>
        </div>
      ) : (
        <div className="space-y-3">
          {carts.map((cart: AdminCartUser) => (
            <div key={cart.user_id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex items-center space-x-6">
                  <div className="text-right">
                    <p className="text-sm text-slate-500">{cart.total_items} items</p>
                    <p className="font-semibold text-slate-800">${cart.cart_total.toFixed(2)}</p>
                  </div>
