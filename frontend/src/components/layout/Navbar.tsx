    ...
    <nav className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            ...
            <div className="hidden md:flex items-center ml-10 space-x-8">
              <Link to="/" className="text-slate-600 hover:text-indigo-600 transition font-medium">
                Home
              </Link>
              <Link to="/products" className="text-slate-600 hover:text-indigo-600 transition font-medium">
                Products
              </Link>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            {user ? (
              ...
              <div className="flex items-center space-x-3">
                <Link
                  to="/login"
                  className="px-4 py-2 text-slate-600 hover:text-indigo-600 font-medium transition"
                >
                  Login
                </Link>
                <Link
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-3">
          <Link to="/" onClick={() => setMobileMenuOpen(false)} className="block text-slate-600 hover:text-indigo-600 font-medium">Home</Link>
          ...
          {user ? (
            ...
            <>
              <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="block text-slate-600 hover:text-indigo-600 font-medium">Login</Link>
              <Link to="/register" onClick={() => setMobileMenuOpen(false)} className="block text-indigo-600 font-medium">Register</Link>
            </>
          )}
        </div>
      )}
